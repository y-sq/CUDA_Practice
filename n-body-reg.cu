...
{
	double local_x, local_y, local_z, local_m;
	...
	for (int start = 0, k = 0; k < BLOCK_NUM; ++k, start += THREAD_NUM) { 
		...
		for (int i = 0; i < THREAD_NUM; ++i) {
			...
			diff_x = __shfl_sync(0xFFFFFFFF, local_x, i, THREAD_NUM) - this_x;
			diff_y = __shfl_sync(0xFFFFFFFF, local_y, i, THREAD_NUM)  - this_y;
			diff_z = __shfl_sync(0xFFFFFFFF, local_z, i, THREAD_NUM)  - this_z;
			...
			coef = inv_square_root * inv_square_root * inv_square_root * __shfl_sync(0xFFFFFFFF, local_m, i, THREAD_NUM) ;
